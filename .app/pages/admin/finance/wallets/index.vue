...
watch([filter, perPage, page, selectedType], fetchWallets)

// Update the router's query parameters whenever filter, perPage, or page changes
watch([filter, perPage, page, selectedType], () => {
  router.push({
    query: {
      ...route.query,
      filter:
  ...
  <div>
    <TairoContentWrapper>
      ...
      <div>
        ...
        >
          <BaseCard
            :class="`grid gap-4 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-${numberOfFiltersToShow} lg:grid-cols-${numberOfFiltersToShow} mb-6 p-5`"
            v-if="filters"
          >
