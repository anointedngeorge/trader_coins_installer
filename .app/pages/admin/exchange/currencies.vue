...
const { handleSubmit, isSubmitting, values, resetForm, setFieldValue } =
  useForm({
...
const update = handleSubmit(async (values: any) => {
  const response = await currencyStore.updateCurrency(
...
const openEditModal = (item: any) => {
  ...
  setFieldValue('fee', item.fee)
  setFieldValue('minWithdraw', item.minWithdraw)
    ...
    <TairoModal :open="isEditOpen" size="sm" @close="closeEditModal">
      ...
      <div class="p-4 md:p-6">
        <div class="mx-auto w-full text-start">
          ...
          >
            {{ $t('Please update the currency information') }}.
          </p>
          <Field
