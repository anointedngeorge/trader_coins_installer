              ...
              >
                ...
                <template #end>
                  <TableFlexTableStart
                    label="Author"
                    :hide-label="index > 0"
                    :logo="item.author?.user?.avatar"
                    :title="`${item.author?.user?.first_name} ${item.author?.user?.last_name}`"
                    :subtitle="item.author?.user?.uuid"
                  />
                  <TableFlexTableCell
                    label="Category"
                    :hide-label="index > 0"
                  ...
                  <TableFlexTableCell
                    label="Tags"
                    :hide-label="index > 0"
                    class="w-30 xs:w-full xs:h-16"
                  >
                    <div
                      v-if="item.post_tag?.length > 0"
                      class="flex flex-col gap-2 overflow-y-auto overflow-x-hidden max-h-16 pr-2 bg-gray-100 dark:bg-gray-900 p-1 rounded"
                    >
                      <BaseTag
                        v-for="(e, index) in item.post_tag"
                        :key="index"
                        flavor="pastel"
                        condensed
                        >{{ e.tag.name }}</BaseTag
                    ...
                    <span v-else class="text-muted-400">{{ $t('N/A') }}</span>
                  </TableFlexTableCell>
                  <TableFlexTableCell
                    label="Status"
                    :hide-label="index > 0"
