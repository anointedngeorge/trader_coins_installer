...
import forexJson from '~~/assets/lottie/forex.json'
import blockchain1Json from '~~/assets/lottie/blockchain1.json'
definePageMeta({
  ...
  <div class="relative">
    ...
    >
      ...
      >
        <client-only>
          <Vue3Lottie
            class="pointer-events-none absolute -top-6 start-3 sm:-start-5 sm:-top-8"
            :animationData="forexJson"
            height="280px"
          />
        </client-only>
      </div>
    ...
    <div v-if="!accounts?.DEMO?.status || !accounts?.LIVE?.status">
      ...
      >
        <template #image>
          <client-only>
            <Vue3Lottie :animationData="blockchain1Json" />
          </client-only>
        </template>
      </BasePlaceholderPage>
    </div>
