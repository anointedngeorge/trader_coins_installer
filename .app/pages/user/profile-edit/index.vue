...
const initialValues = computed<FormInput>(() => ({
  ...
  metadata: {
    location: {
      ...
      zip: user.metadata?.location?.zip || '',
    },
    ...
    info: {
      experience: [
        ...
        '5-10 years',
        '10+ years',
      ].includes(user.metadata?.info?.experience)
        ? user.metadata?.info?.experience
        : null,
      firstJob: user.metadata?.info?.firstJob || null,
      flexible: user.metadata?.info?.flexible || null,
      remote: user.metadata?.info?.remote || null,
    },
    social: {
      facebook: user.metadata?.social?.facebook || '',
      twitter: user.metadata?.social?.twitter || '',
      dribbble: user.metadata?.social?.dribbble || '',
      instagram: user.metadata?.social?.instagram || '',
      github: user.metadata?.social?.github || '',
      gitlab: user.metadata?.social?.gitlab || '',
    },
  },
}))
...
const inputFile = ref<FileList | null>(user.avatar || null)
...
const handleFileUpload = async () => {
  ...
  body.append('oldImagePath', user.avatar)
