...
async function withdraw() {
  ...
  try {
    ...
    if (response.status === 'success') {
      router.push(`/user/wallets/spot/${currency.toLowerCase()}`)
    }
  } catch (e) {
    toast.danger(e)
  }
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <div>
          <BaseButton
            :to="`/user/wallets/spot/${currency.toLowerCase()}`"
            color="muted"
            flavor="pastel"
          >
          ...
          <div class="space-y-5 pl-8">
            ...
            <div class="w-full">
              <BaseInput
                ...
                :key="selectedChain"
                :min="Number(currencyChainData?.withdrawMin)"
                :max="Number(currencyChainData?.withdrawMax)"
                shape="rounded"
                type="number"
                ...
                :error="
                  withdrawAmount < Number(currencyChainData?.withdrawMin) ||
                  withdrawAmount > Number(currencyChainData?.withdrawMax)
                    ? `Withdraw amount must be between ${currencyChainData?.withdrawMin} ${currency} and ${currencyChainData?.withdrawMax} ${currency}`
                    : false
                "
              />
            </div>
