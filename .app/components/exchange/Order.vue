...
async function storeOrder() {
  ...
  // Validations for STOP_LIMIT type
  // if (isType('STOP_LIMIT')) {
  //   if (marketStore.order.stopLossPrice === 0) {
  //     toast.dangertext('Please enter a stop loss price')
  //     return
  //   }
  //   if (
  //     side.value === 'BUY' &&
  //     marketStore.order.stopLossPrice >= bestAsk.value
  //   ) {
  //     toast.dangertext(
  //       'Stop loss price must be below the current market price for a BUY order',
  //     )
  //     return
  //   }
  ...
  //   if (
  //     side.value === 'SELL' &&
  //     marketStore.order.stopLossPrice <= bestBid.value
  //   ) {
  //     toast.dangertext(
  //       'Stop loss price must be above the current market price for a SELL order',
  //     )
  //     return
  //   }
  ...
  //   if (
  //     side.value === 'BUY' &&
  //     marketStore.order.price < marketStore.order.stopLossPrice
  //   ) {
  //     toast.dangertext(
  //       'Limit price must be higher or equal to the stop loss price for a BUY order',
  //     )
  //     return
  //   }
  ...
  //   ) {
  //     toast.dangertext(
  //       'Limit price must be lower or equal to the stop loss price for a SELL order',
  //     )
  //     return
  //   }

  //   if (marketStore.order.takeProfitPrice === 0) {
  //     toast.dangertext('Please enter a take profit price')
  //     return
  //   }

  //   if (
  //     side.value === 'BUY' &&
  //     marketStore.order.takeProfitPrice <= bestBid.value
  //   ) {
  //     toast.dangertext(
  //       'Take profit price must be above the current market price for a BUY order',
  //     )
  //     return
  //   }
  ...
  //   if (
  //     side.value === 'SELL' &&
  //     marketStore.order.takeProfitPrice >= bestAsk.value
  //   ) {
  //     toast.dangertext(
  //       'Take profit price must be below the current market price for a SELL order',
  //     )
  //     return
  //   }
  ...
  //   if (
  //     side.value === 'BUY' &&
  //     marketStore.order.price > marketStore.order.takeProfitPrice
  //   ) {
  //     toast.dangertext(
  //       'Limit price must be lower or equal to the take profit price for a BUY order',
  //     )
  //     return
  //   }
  ...
  //   ) {
  //     toast.dangertext(
  //       'Limit price must be higher or equal to the take profit price for a SELL order',
  //     )
  //     return
  //   }
  // }
  ...
  const order = {
    ...
    fee: totalFees.value,
    fee_currency: side.value === 'BUY' ? props.currency : props.pair,
    cost: totalCost.value,
    symbol: `${props.currency}/${props.pair}`,
    // stopLossPrice: marketStore.order.stopLossPrice || null,
    // takeProfitPrice: marketStore.order.takeProfitPrice || null,
  }
  ...
  try {
    ...
    if (response.status === 'success') {
      ...
      await walletStore.fetchWallets()
      // marketStore.order.stopLossPrice = null
      // marketStore.order.takeProfitPrice = null
    }
  } catch (error) {
...
<template>
  <div
    ...
    :class="{
      ...
      'h-2/3': showOrderContent && $viewport.isLessThan('sm'),
    }"
  >
    <div
      ...
      >
        ...
        >
          ...
          <div class="flex justify-between px-2 text-xs" key="fees">
            <span class="text-gray-500 dark:text-gray-400"
              >{{ side === 'BUY' ? 'Taker' : 'Maker' }} {{ $t('Fees') }} ({{
                side === 'BUY' ? takerFee : makerFee
              }}%)</span
            >
            <span class="text-gray-700 dark:text-gray-200" :key="side"
              >{{ totalFees.toFixed(amountPrecision) }}
              {{ side === 'BUY' ? currency : pair }}</span
            >
          </div>
          <div class="flex justify-between px-2 text-xs" key="total">
            ...
            <span class="text-gray-700 dark:text-gray-200" :key="side"
              >{{ totalAmount.toFixed(amountPrecision) }}
              {{ side === 'BUY' ? currency : pair }}</span
            >
          </div>
          <div class="flex justify-between px-2 text-xs" key="total">
