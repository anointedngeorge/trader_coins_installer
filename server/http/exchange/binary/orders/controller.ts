...
function setupPendingOrdersCron() {
  CronManager.addJob('process pending orders', '0 * * * *', async () => {
    try {
      await processPendingOrders()
    } catch (error) {
      console.error(`Error running process pending orders task: ${error}`)
    }
  })

  CronManager.startJob('process pending orders')
}

setupPendingOrdersCron()
