...
function setupFiatCurrenciesCron() {
  CronManager.addJob('fetch fiat currency prices', '0 * * * *', async () => {
    try {
      await fetchFiatCurrencyPrices()
    } catch (error) {
      console.error(`Error running fetch fiat currency prices task: ${error}`)
    }
  })

  CronManager.startJob('fetch fiat currency prices')
}

setupFiatCurrenciesCron()
